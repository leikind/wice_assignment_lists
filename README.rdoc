...
Wice Assignment Lists makes it dead easy to create a widget consisting of two multiple selection lists with
...
Wice Assignment Lists works currently only under Prototype.

Wice Assignment Lists works with Rails 2.3.8 with or without rails_xss.
...
Routes:
  ...
  map.resource :roles, :member => {:filter => :post}
...
Controller :
  ...
  class RolesController < ActionController::Base
    ...
    def update
      @adminstrator_role = Role.find(:first)
      @adminstrator_role.users = User.find(params[:users])
...
Run the following to copy the javascript, the stylesheet, and the icons to public, and the configuration file to <tt>config/initializers</tt>:
